...
//            }
    ...
    public List<Integer> getRevenueData() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            String targetDateString = targetYearMonth.format(formatter);

            // Prepare SQL query to sum total_price for orders in the target month
            String query = "SELECT COALESCE(SUM(total_price), 0) AS revenue "
                    + "FROM Orders "
                    + "WHERE YEAR(order_date) = ? AND MONTH(order_date) = ?";
            try {
                PreparedStatement ps = connection.prepareStatement(query);
                ps.setInt(1, targetYearMonth.getYear());
                ps.setInt(2, targetYearMonth.getMonthValue());
                ResultSet rs = ps.executeQuery();
                if (rs.next()) {
                    int revenue = rs.getInt("revenue");
                    revenueList.add(revenue);
                } else {
                    // If no revenue data for the month, add 0
                    revenueList.add(0);
                }
            } catch (SQLException e) {
                e.printStackTrace();
        ...
        // Reverse the revenue list to have the current month first
    ...
    public List<Integer> getOrderCountData() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            // Prepare SQL query to count orders in the target month
            String query = "SELECT COALESCE(COUNT(*), 0) AS order_count "
                    + "FROM Orders "
                    + "WHERE YEAR(order_date) = ? AND MONTH(order_date) = ?";
            try {
                PreparedStatement ps = connection.prepareStatement(query);
                ...
                if (rs.next()) {
                    ...
                    // If no order count data for the month, add 0
        ...
        // Reverse the order count list to have the current month first
